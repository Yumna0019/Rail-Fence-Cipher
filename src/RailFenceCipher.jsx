...
//     let pattern = new Array(str.length).fill(0);
//     let row = 0, down = false;
...
//       for (let i = 0; i < str.length; i++) {
//         if (pattern[i] === r) rails[r].push(str[index++]);
//       }
//     }
...
//     for (let i = 0; i < str.length; i++) {
//       decoded += rails[pattern[i]][railPointers[pattern[i]]++];
//     }
...
const RailFenceCipher = () => {
  ...
  const decodeRailFence = (str, numRails) => {
    ...
    // Step 1: Create the zigzag pattern array
    ...
    // Step 2: Allocate characters to rails
    let rails = Array.from({ length: numRails }, () => []);
    let index = 0;

    // Fill each rail with the correct characters
    for (let r = 0; r < numRails; r++) {
        for (let i = 0; i < str.length; i++) {
            if (pattern[i] === r) {
    ...
    // Step 3: Read the characters in the original zigzag order
    let decoded = "";
    let railPointers = new Array(numRails).fill(0);
